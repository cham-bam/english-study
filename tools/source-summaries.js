...
const ENGLISH_GENERATION_VERSION = "dictionary-source-v2";
const SPEAKING_GENERATION_VERSION = "source-speaking-v3";
...
const KNOWLEDGE_SOURCES = [
  {
    ...
    body: "물은 증발, 응결, 강수, 지표 흐름, 지하수 흐름을 거치며 지구 안에서 계속 위치를 바꿉니다. 첫째, 이 과정은 바다와 대기와 땅을 연결해 기후와 날씨를 움직입니다. 둘째, 강과 지하수는 생태계와 사람이 쓸 물을 옮기는 통로가 됩니다. 셋째, 물은 사라지는 것이 아니라 상태와 위치가 바뀌기 때문에 오염과 사용 습관도 결국 순환 안에서 되돌아옵니다.",
  ...
  },
  {
    ...
    body: "수면은 에너지를 아끼는 시간이 아니라 뇌와 몸이 회복 작업을 하는 시간입니다. 첫째, 충분한 수면은 기억과 학습에 관여합니다. 둘째, 심장, 호르몬, 면역 기능처럼 몸의 기본 조절에도 영향을 줍니다. 셋째, 잠이 부족하면 집중력과 감정 조절이 흔들려 다음 날 판단 품질까지 떨어질 수 있습니다.",
  ...
  },
  {
    ...
    body: "음식을 안전하게 보관하려면 온도, 시간, 오염 가능성을 함께 봐야 합니다. 첫째, 차갑게 보관하면 미생물 증식 속도를 낮출 수 있습니다. 둘째, 익힌 음식과 날음식을 분리하면 교차오염 위험이 줄어듭니다. 셋째, 남은 음식은 오래 두는 것보다 빨리 식히고 밀폐해 보관하는 편이 안전합니다.",
  ...
  },
  {
    ...
    body: "재활용은 버리는 행동이 아니라 다시 쓸 수 있는 재료로 준비하는 과정입니다. 첫째, 음식물과 액체가 남아 있으면 재활용품 전체 품질이 떨어집니다. 둘째, 지역마다 처리 가능한 재질이 다르기 때문에 표시와 지역 기준을 확인해야 합니다. 셋째, 재사용과 감량은 재활용보다 앞단에서 폐기물 자체를 줄이는 방법입니다.",
  ...
  },
  {
    ...
    body: "AI는 정확도만으로 평가하기 어렵고 위험을 식별하고 줄이는 절차가 함께 필요합니다. 첫째, 데이터 편향이나 부정확한 출력은 실제 사람의 판단과 권리에 영향을 줄 수 있습니다. 둘째, 모델이 어떤 상황에서 실패하는지 기록하고 평가해야 합니다. 셋째, 조직은 AI의 책임자, 검토 기준, 사용 제한을 정해야 신뢰 가능한 도입이 가능합니다.",
  ...
  },
  {
    ...
    body: "분산투자는 여러 자산에 나누어 투자해 특정 위험에 덜 흔들리도록 만드는 원칙입니다. 첫째, 한 자산의 손실이 전체 자산을 한 번에 무너뜨리는 것을 줄입니다. 둘째, 서로 다르게 움직이는 자산을 섞을수록 충격 흡수 효과가 커질 수 있습니다. 셋째, 분산은 손실을 없애는 기술이 아니라 과도한 집중을 피하는 위험 관리입니다.",
  ...
  },
  {
    ...
    body: "집을 살 수 있는지는 매매가 하나가 아니라 대출 조건과 월 지출 전체로 판단해야 합니다. 첫째, 금리가 바뀌면 같은 가격의 집도 월 납입액이 크게 달라집니다. 둘째, 세금, 보험, 관리비, 유지보수비가 실제 주거비에 포함됩니다. 셋째, 무리한 대출은 가격 상승 기대가 맞아도 생활 현금흐름을 압박할 수 있습니다.",
  ...
  },
  {
    ...
    body: "전기차는 배터리와 모터만의 문제가 아니라 충전 환경과 사용 패턴의 문제입니다. 첫째, 집이나 직장에서 충전할 수 있으면 일상 사용의 불편이 크게 줄어듭니다. 둘째, 주행거리와 충전 속도는 장거리 이동에서 중요해집니다. 셋째, 전기차의 비용 판단은 구매가뿐 아니라 에너지 비용과 유지관리 비용까지 함께 봐야 합니다.",
  ...
  },
  {
    ...
    body: "모델이 훈련 데이터의 세부 패턴을 지나치게 외우면 새 데이터에서 성능이 떨어질 수 있습니다. 첫째, 훈련 점수만 높다고 좋은 모델이라고 말할 수 없습니다. 둘째, 검증 데이터와 테스트 데이터는 모델이 일반화하는지 확인하는 장치입니다. 셋째, 단순화, 정규화, 더 나은 데이터 분할은 과적합을 줄이는 데 쓰입니다.",
  ...
  },
  {
    ...
    body: "자동 긴급제동, 차선 유지, 경고 시스템 같은 기술은 운전자를 없애기보다 위험을 빠르게 감지하고 반응 시간을 줄이는 데 초점이 있습니다. 첫째, 센서는 사람이 놓칠 수 있는 거리와 속도 변화를 감지합니다. 둘째, 경고와 보조 기능은 사고 전 몇 초의 대응 여지를 늘립니다. 셋째, 기술이 있어도 운전 책임과 시스템 한계를 이해해야 안전 효과가 커집니다.",
  ...
  },
  {
    ...
    body: "암호화는 데이터를 정해진 규칙과 열쇠로 바꾸어 허가된 사람만 읽게 하는 기술입니다. 첫째, 온라인 결제와 메시지 보호는 암호 기술 위에서 작동합니다. 둘째, 안전한 시스템은 알고리즘뿐 아니라 키 관리와 인증 절차가 함께 필요합니다. 셋째, 암호화는 데이터를 완전히 안전하게 만드는 마법이 아니라 공격 비용을 높이고 접근 권한을 통제하는 방법입니다.",
  ...
  },
  {
    ...
    body: "자율주행은 모든 상황에서 사람을 바로 대체하는 기술이라기보다 자동화 수준과 책임 범위를 구분해야 하는 분야입니다. 첫째, 평범한 차선 유지보다 공사 구간, 악천후, 돌발 보행자 같은 예외가 어렵습니다. 둘째, 시스템이 언제 운전자 개입을 요구하는지 명확해야 합니다. 셋째, 안전 검증은 기술 성능뿐 아니라 책임, 규제, 사용자의 이해를 함께 요구합니다.",
...
let englishSourceCache = null;

function loadEnglishSourceWords() {
  if (englishSourceCache) return englishSourceCache;

  const sourceFile = path.resolve(__dirname, "..", "01_기초_동사형용사.json");
  try {
    const raw = JSON.parse(fs.readFileSync(sourceFile, "utf8"));
    const words = Array.isArray(raw) ? raw : Array.isArray(raw.words) ? raw.words : [];
    englishSourceCache = words
      .filter((item) => item?.word && item?.meaning && item?.example && DIFFICULTIES.includes(item?.difficulty))
      .map((item) => ({
        ...
        sourceDay: item.day || null
      }));
  } catch {
    englishSourceCache = [];
  }

  return englishSourceCache;
}

function fallbackEnglishWords() {
  return DIFFICULTIES.flatMap((difficulty) => (ENGLISH_WORDS[difficulty] || []).map((item) => ({
    difficulty,
    word: item[0],
    meaning: item[1],
    example: item[2],
...
function englishCatalog(difficulty) {
  const sourceWords = loadEnglishSourceWords();
  const words = sourceWords.length ? sourceWords : fallbackEnglishWords();
  return words.filter((item) => item.difficulty === difficulty).map((item) => ({
    difficulty: item.difficulty,
    word: item.word,
    meaning: item.meaning,
    example: item.example,
    ...
    sources: [source("Oxford Learner's Dictionaries", oxfordUrl(item.word))]
  }));
}
...
function asReviewItem(item) {
  return {
    ...item,
    note: `일요일 복습 단어입니다. 어렵게 느꼈거나 최근에 다시 확인할 가치가 있는 단어로, 예문을 먼저 읽고 뜻을 떠올려보세요.`
  };
}

function makeReviewEnglish(date, quota, options) {
  const seed = dateSeed(date);
  const picked = [];

  DIFFICULTIES.forEach((difficulty, difficultyIndex) => {
    const reviewWords = keyedWords([
      ...(options.reviewWordsByDifficulty?.[difficulty] || []),
      ...(options.recentWordsByDifficulty?.[difficulty] || [])
    ]);
    const fallback = englishCatalog(difficulty);
    const candidates = rotate(reviewWords.length ? reviewWords : fallback, seed + difficultyIndex * 17);
...
function makeGeneratedEnglish(date, quota, usedWords, options = {}) {
  if (isSunday(date)) {
    return makeReviewEnglish(date, quota, options);
  }
  ...
  DIFFICULTIES.forEach((difficulty, difficultyIndex) => {
    ...
    for (const item of candidates) {
      ...
      picked.push({
        ...item,
        note: `Oxford Learner's Dictionaries 출처로 확인하는 신규 단어입니다. 최근 1주일과 전체 기존 출제 이력에서 제외한 단어로 골랐습니다.`
      });
      pickedWords.add(key);
      count += 1;
    }
...
const SPEAKING_SOURCES = [
  ...
  },
  {
    ...
    bodyKo: "집값 하나만 보고 구매 가능성을 판단하면 실제 부담을 놓치기 쉽습니다. 첫째, 금리와 대출 기간은 같은 매매가의 집도 완전히 다른 월 납입액으로 바꿉니다. 둘째, 세금, 보험, 관리비, 수리비는 매달 생활비와 함께 나가는 비용입니다. 셋째, 집값 상승 기대가 있더라도 현금흐름이 버티지 못하면 생활 안정성이 흔들립니다. 1분 설명에서는 '가격보다 월 부담액, 월 부담액보다 지속 가능한 현금흐름' 순서로 말하면 구조가 잡힙니다.",
  ...
  },
  {
    ...
    bodyKo: "전기차 선택은 배터리 용량이나 주행거리만으로 끝나지 않습니다. 첫째, 집이나 직장에서 충전할 수 있는 사람은 일상 사용의 불편이 크게 줄어듭니다. 둘째, 장거리 이동에서는 충전기의 위치뿐 아니라 작동 여부, 충전 속도, 대기 시간이 중요합니다. 셋째, 전기차 비용은 구매가와 연료비, 유지비를 함께 보는 총소유비용 관점으로 이해해야 합니다. 1분 설명에서는 전기차의 다음 경쟁력이 '차량 성능 + 충전 생활권'이라고 정리하면 됩니다.",
  ...
  },
  {
    ...
    bodyKo: "운전자 보조 기술은 운전자를 완전히 대체하기보다 위험 순간을 줄이는 보조 체계입니다. 첫째, 센서와 경고는 사람이 놓칠 수 있는 거리와 속도 변화를 빠르게 잡아냅니다. 둘째, 자동 긴급제동이나 차선 유지 기능은 사고 직전의 반응 시간을 늘립니다. 셋째, 기술의 한계를 모르면 운전자가 시스템을 과신해 오히려 위험해질 수 있습니다. 1분 설명에서는 '자동차 AI는 만능 조종사가 아니라 위험을 줄이는 보조자'라고 말하면 핵심이 잡힙니다.",
  ...
  },
  {
    ...
    bodyKo: "AI 건물 관리는 설비 데이터를 통해 이상 신호를 더 빨리 찾는 방향으로 발전합니다. 첫째, 온도, 전력 사용, 진동, 출입 패턴 같은 데이터는 설비 상태의 단서가 됩니다. 둘째, 고장을 예측하면 입주자 불편과 운영 중단을 줄일 수 있습니다. 셋째, 건물 데이터는 사생활과 보안 문제를 만들 수 있어 수집 범위와 접근 권한을 명확히 해야 합니다. 1분 설명에서는 '효율과 신뢰를 동시에 설계해야 하는 운영 기술'이라고 정리하면 좋습니다.",
  ...
  },
  {
    ...
    bodyKo: "AI의 가치는 멋진 답변보다 반복 업무의 병목을 줄일 때 분명해집니다. 첫째, 생산, 물류, 고객 응대 중 시간이 가장 많이 막히는 지점을 찾아야 적용 지점이 보입니다. 둘째, 오류 비용이 큰 단계일수록 작은 자동화도 큰 효과를 낼 수 있습니다. 셋째, 데이터가 정리되어 있지 않으면 좋은 모델도 현장 개선으로 이어지기 어렵습니다. 1분 설명에서는 '산업 AI는 모델 도입이 아니라 병목 정의와 성과 측정의 문제'라고 말하면 됩니다.",
...
function expandSpeakingSummaryKo(item) {
  return `${item.bodyKo} 1분 설명은 세 단계로 잡으면 좋습니다. 먼저 핵심 변화나 문제를 한 문장으로 말하고, 다음으로 실제 생활이나 산업에서 바뀌는 행동을 연결합니다. 마지막으로 최신 수치와 개별 사례는 출처를 다시 확인해야 한다고 덧붙이면 설명이 단단해집니다.`;
}

function expandSpeakingSummaryEn(item) {
  return `${item.bodyEn} To explain this in your own words, use three steps. Start with the core change or problem in one sentence. Then connect it to a practical shift in daily life or industry. Finally, add that current numbers and individual cases should be checked against the source.`;
}
...
const CARE_SOURCES = [
  {
    ...
    action: "관계의 신뢰는 큰 이벤트보다 작은 관심 요청에 어떻게 반응하는지에서 자주 쌓입니다. 상대가 말을 걸거나 피곤함을 비칠 때 휴대폰을 잠깐 내려놓고 눈을 맞추는 행동이 작은 신호에 응답하는 방식입니다.",
  ...
  },
  {
    ...
    action: "건강한 관계에서는 상대를 비난하기보다 자신의 감정과 필요를 명확히 말하는 방식이 중요합니다. '너는 항상 그래'보다 '나는 이 상황에서 서운했고 다음에는 이렇게 해주면 좋겠어'가 더 안전한 대화입니다.",
  ...
  },
  {
    ...
    action: "좋은 관계는 긍정적 상호작용을 자주 쌓는 쪽으로 강해집니다. 상대가 해준 일을 그냥 넘기지 말고 무엇이 편했고 왜 고마웠는지 구체적으로 말해 주세요.",
  ...
  },
  {
    ...
    action: "갈등 후에는 '미안해'에서 멈추지 않고 다음 행동을 말해야 신뢰가 회복됩니다. 인정, 영향 이해, 다음 행동을 짧게 연결하세요.",
  ...
  },
  {
    ...
    action: "관계의 온도는 거창한 이벤트보다 자주 반복되는 작은 행동에서 유지됩니다. 추운 날 실내 동선을 잡거나, 피곤한 날 이동을 줄여주는 식의 행동이 실제 체감 배려입니다.",
  ...
  },
  {
    ...
    action: "존중은 상대의 경계와 선택권을 인정하는 데서 시작됩니다. 사진을 올리거나 일정을 정하거나 누군가에게 소개할 때는 내 의도가 좋아도 먼저 물어보는 편이 안전합니다.",
